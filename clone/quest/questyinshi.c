...
int ask_me(object who)
{
        ...
        else message_vision(HIG"$N叹了口气，看着$n苦笑道：“你这么辛苦地找我，所为何事？”。\n"NOR, ob, me);
        return 1;
}
...
int random_move()
{
		...
		//lijia
        if( dest->query("no_fight")) return 0;
      ...
      if( ob->query("ask_time")<3)
          {
            message_vision(msg_dunno[random(sizeof(msg_dunno))], me, ob);           
             ...
             ||ob->query("family_name") != me->query("family/family_name"))
          {
             return  notify_fail( ob ->name() + reject_msg[random(sizeof(reject_msg))] );
          }   
      else{     
           if (!arg) return notify_fail("你要请教什么？\n");
...
void die()
{
        ...
        fme=query_temp("last_opponent");          
         ...
         if( fme && 
             ob->query("ask_time")>3&& ob->query("owner")==me->query("id")
             && ob->query("owner")==fme->query("id")
             && ob->query("family_name") != me->query("family/family_name"))
            {
              pot = 400+random(400);
              exp = 1500+random(500);
