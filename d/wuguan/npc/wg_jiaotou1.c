...
void create()
{
	set_name("王教头", ({ "wang jiaotou", "wang","jiaotou"}));
...
int accept_object(object who, object ob)
{
    ...
    {
		message_vision("王教头望着$N说：你的武功应该历练江湖才能再长进，不能埋没
在这里了。\n", who);
		return 0;
    }
    if (ob->query("money_id") && ob->value() >= 500)
    {
		...
		message_vision("王教头对$N说：好！这位" + RANK_D->query_respect(who) 
+ "想学什么呢？\n" , who);
		return 1;
    }
    else
		message_vision("王教头皱眉对$N说：钱我不在乎。可你也给的也太少了点儿吧？\n", who);
	return 0;
}
