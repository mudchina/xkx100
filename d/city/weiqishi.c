...
{
	if (arg=="book")
		this_player()->start_more(read_file("/d/changan/obj/go_book"));
	if (arg=="jue")
...
int no_qi(int * bd, int x, int y)
{
	...
	while(bindex>0)
	{
		bd[blist[bindex-1]] = mycolor; bindex--;
	}
	return(!qi_flag);
}
...
int weiqi_rule(int x, int y)
{
	...
	if(n>1)
	{
		...
		{
			if(jie_flag==WQ_JIE_POSSIBLE && x==jie_x_ban &&
				y==jie_y_ban)
