...
int zhuang_deposit = 500; //5 gold. 当庄保证金
...
int do_zuozhuang()
{
	...
	{//only in this two case can 换庄
		...
		else
		{
			...
			me->set_temp("shaizi_zhuang_deposit", zhuang_deposit);
...
void pay_player(object me,int total)
{
	MONEY_D->pay_player(me,total*100);
}
int pay_out(object me,int total)
{
	return MONEY_D->player_pay(me,total*100);
}
/*void pay_player(object me, int total)
{
...
//	cash = present("1000-cash_money", me); // 这里的1000-cash_money不对
