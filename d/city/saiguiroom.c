...
int do_sai(string arg)
{
	...
	message_vision(sprintf("$N拿出%s%s%s放在紫檀龟台上，押%s。\n", chinese_number(amount), ob->query("base_unit"), ob->query("name"), guis[what]), me);
...
void gamble_finish()
{
	...
	else if ((res[0]==30 && res[1]==30) || (res[1]==30 && res[2]==30) ||
			 (res[2]==30 && res[0]==30))
	{
		total = 0;
		win = "none  of  them";
