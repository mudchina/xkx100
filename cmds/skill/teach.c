...
int main(object me, string arg)
{
	...
	if (environment(me)->is_chat_room()) return notify_fail("这里禁止练功。\n");
	...
	if( (int)me->query("jing") > gin_cost ) {
		...
		tell_object(ob,"但是"+me->name()+"显然太累了，没有办法教你什么。\n");
		return 1;
	}
...
指令格式 : teach | jiao <某人> <技能>
    ...
    这个指令可以让你向你的伴侣解释有关某一种技能的疑难问题，当
然，你的伴侣在这项技能上的造诣必须比你低，而你的伴侣经由这种方
式学习得来的技能也不可能高于你，然而因为这种传授方式相当於一种
「经验的传承」，因此这可以说是熟悉一种新技能较快的方法。

    此外学习也需要消耗一些精气，而消耗的精气跟你自己、与你的伴
侣的悟性，以及二
人的恩爱程度有关。
