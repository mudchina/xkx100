...
int accept_reply(object me,object ob,string arg)
{
  ...
  if (living(me) && living(ob) && ans==1)
  {
		...
		if ( mysc < me->query_temp("quest_gift/need") )
			return notify_fail("你就这么点"+need+"，也好意思问"+ob->name()+"要东西？\n");
      message_vision(CYN "\n$N" CYN "急忙点点头，说道：这东西正好我也需要，就交给我吧。\n" NOR, me);
      arg=me->query_temp("quest_gift/obj");
      obj = new(arg);
      if (!objectp(obj))
        return notify_fail(ob->name()+"纳闷道：咦？我的东西呢？刚才还在，怎么转眼就没了。\n");
      ...
      message_vision("$N微微一笑，拿出一" + un + obj->name() + NOR "交给$n。\n" NOR,ob,me);
      obj->move(me);
      ...
      {
        me->delete_temp("quest_gift/got");
        tell_object(me,HIW+ob->name()+"叹气道：也罢，既然你不需要，这东西我"
                                       "就留给别人吧。\n"NOR);
        return 1;
      }
      return 0;
...
//    destruct(ob);
  ...
  else
  {
    ...
    if (num>49 && num<501 && num%50==0 ) 
      special_bonus(this_object(),who,"meili");  //整五十做特殊物品奖励
     ...
     //     GIFT_DIR+"gift/int2",
       ...
       //   GIFT_DIR+"gift/int3",
        ...
        if (mysc >= cost)
        {
                ...
                else if (type = "gongxian") need = "门派贡献";
                tell_object(who, HIW "\n获得" + ob->name() + HIW "会降低"
                                 "你" + chinese_number(cost) + "点"+need+
                                 "，你可以选择回答(reply Y|N)是或者否。\n" NOR);
